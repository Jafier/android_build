...
- jrremote: Add git remote for JR Gerrit Review.
- jafgerrit: A Git wrapper that fetches/pushes patch from/to JR Gerrit Review.
- jrrebase: Rebase a Gerrit change and push it again.
- aospremote: Add git remote for matching AOSP repository.
- cafremote: Add git remote for matching CodeAurora repository.
- mka:      Builds using SCHED_BATCH on all processors.
- mkap:     Builds the module(s) using mka and pushes them to the device.
- jrka:     Cleans and builds using mka.
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^jr_") ; then
       JR_BUILD=$(echo -n $1 | sed -e 's/^jr_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $JR_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       JR_BUILD=
    fi
    export JR_BUILD
...
{
    target=$1
    JR_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/jr/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the JR model name
            lunch jr_$target-userdebug
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the JR github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var JR_VERSION)
        ZIPFILE=jr-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.jr.device=$JR_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $JR_BUILD, run away!"
    fi
}
...
function jrremote()
{
    git remote rm jrremote 2> /dev/null
    if [ ! -d .git ]
    then
    ...
    JRUSER=`git config --get review.gerrit.jafierroms.com.username`
    if [ -z "$CMUSER" ]
    then
        git remote add jrremote ssh://gerrit.jafierroms.com:29419/$GERRIT_REMOTE
    else
        git remote add jrremote ssh://$JRUSER@gerrit.jafierroms.com:29419/$GERRIT_REMOTE
    fi
    echo You can now push to "jrremote".
}
...
function makerecipe() {
  ...
  then
    pwd
    jrremote
    git push jrremote HEAD:refs/heads/'$1'
  fi
  '
...
function jafgerrit() {
    ...
    local user=`git config --get review.gerrit.jafierroms.com.username`
...
function jrrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "JafierRoms Gerrit Rebase Usage: "
        echo "      jrrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://gerrit.jafierroms.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/jr/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
